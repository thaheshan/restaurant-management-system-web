...
import { FiPlus, FiEdit2, FiTrash2, FiX, FiRefreshCw, FiTag, FiShoppingBag, FiCheckCircle, FiAlertTriangle } from 'react-icons/fi';
...
const MenuManagement = () => {
  ...
  const [itemForm, setItemForm] = useState({ name: '', description: '', price: '', ingredients: '', categoryId: '', is_available: true });
  ...
  const openAddCategory = () => { setEditingCategory(null); setCategoryForm({ name: '', description: '' }); setError(''); setShowCategoryModal(true); };
  const openEditCategory = (cat: any) => { setEditingCategory(cat); setCategoryForm({ name: cat.name, description: cat.description || '' }); setError(''); setShowCategoryModal(true); };
  ...
  const handleSaveCategory = async () => {
    ...
    try {
      ...
      const res = await fetch(url, { method: editingCategory ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ restaurantId, name: categoryForm.name, description: categoryForm.description }) });
      const data = await res.json();
  ...
  const openAddItem = (categoryId = '') => { setEditingItem(null); setItemForm({ name: '', description: '', price: '', ingredients: '', categoryId, is_available: true }); setError(''); setShowItemModal(true); };
  const openEditItem = (item: any, categoryId: string) => {
    setEditingItem(item);
    setItemForm({ name: item.name, description: item.description || '', price: String(item.price), ingredients: item.ingredients || '', categoryId, is_available: item.is_available !== false });
  ...
  const handleSaveItem = async () => {
    ...
    try {
      ...
      const res = await fetch(url, { method: editingItem ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ restaurantId, categoryId: itemForm.categoryId, name: itemForm.name, description: itemForm.description, price: parseFloat(itemForm.price), ingredients: itemForm.ingredients, is_available: itemForm.is_available }) });
      const data = await res.json();
  ...
  const cardStyle: React.CSSProperties = { background: 'white', borderRadius: 20, boxShadow: '0 20px 60px rgba(0,0,0,0.2)', width: '100%', maxWidth: 480, overflow: 'hidden' };
            ...
            <div key={cat.id} style={{ background: 'white', borderRadius: 12, boxShadow: '0 1px 6px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: cat.items?.length > 0 ? '1px solid #f5f5f5' : 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  <div style={{ width: 42, height: 42, borderRadius: 10, background: 'rgba(45,90,61,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: P, fontWeight: 800, fontSize: '1rem' }}>
                    ...
                    <div key={item.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', borderRadius: 8, marginTop: 4, background: '#fafafa', border: '1px solid #f0f0f0' }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                          ...
                          <span style={{ padding: '2px 8px', borderRadius: 20, fontSize: '0.68rem', fontWeight: 700, background: item.is_available ? 'rgba(34,197,94,0.1)' : 'rgba(156,163,175,0.15)', color: item.is_available ? '#16a34a' : '#9ca3af' }}>
        ...
        <div style={modalStyle} onClick={() => setShowItemModal(false)}>
          <div style={cardStyle} onClick={e => e.stopPropagation()}>
            ...
            <div style={mBody}>
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600, color: '#444' }}>
                <input type="checkbox" checked={itemForm.is_available} onChange={e => setItemForm({...itemForm, is_available: e.target.checked})} style={{ width: 16, height: 16, accentColor: P }} />
